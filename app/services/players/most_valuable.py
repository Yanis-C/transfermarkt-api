...
@dataclass
class TransfermarktPlayerMostValuable(TransfermarktBase):
    """
    A class for searching most valuable football players on Transfermarkt and retrieving search results.
    ...
    URL: str = (
        "https://www.transfermarkt.com/marktwertetop/wertvollstespieler/?page={page_number}"
    )
    page_number: int = 1

    def __post_init__(self) -> None:
        """Initialize the TransfermarktPlayerMostValuable class."""
        self.URL = self.URL.format(page_number=self.page_number)
        self.page = self.request_url_page()
        self.raise_exception_if_not_found(xpath=Players.MostValuable.FOUND)
    ...
    def __parse_search_results(self) -> list:
        ...
        search_results: list[ElementTree] = self.page.xpath(Players.MostValuable.RESULTS)
        results = []

        for result in search_results:
            rank = trim(result.xpath(Players.MostValuable.RANK))
            idx = extract_from_url(result.xpath(Players.Search.ID))
            name = trim(result.xpath(Players.Search.NAME))
            club_name = trim(result.xpath(Players.MostValuable.CLUB_NAME))
            club_id = safe_regex(result.xpath(Players.MostValuable.CLUB_IMAGE), REGEX_CHART_CLUB_ID, "club_id")
            age = trim(result.xpath(Players.MostValuable.AGE))
    ...
    def search_players(self) -> dict:
        ...
        self.response["lastPageNumber"] = self.get_last_page_number(Players.MostValuable.BASE)
